...
def load_config(config_path: Path | None = None) -> Config:
    ...
    _apply_provider_compat_env(config)
...
def save_config(config: Config, config_path: Path | None = None) -> None:
    ...
    Secrets are written to ~/.kyber/.env (mode 600) and removed from
    config.json so that the JSON file contains no secret fields.
    ...
    # Extract secrets → .env, then remove secret fields from JSON payload
    env_path = get_env_path()
...
def _delete_nested(data: dict, keys: tuple[str, ...]) -> None:
    """Delete a nested key from a dict by key path if it exists."""
    current: Any = data
    for k in keys[:-1]:
...
def _strip_secrets(data: dict) -> None:
    """Remove secret fields from camelCase config data (in-place)."""
    for config_keys in SECRET_PATHS:
        _delete_nested(data, config_keys)
...
def _set_env_if_missing(key: str, value: str) -> None:
    """Set an environment variable only when non-empty and not already set."""
    if not value:
        return
    os.environ.setdefault(key, value)


def _apply_provider_compat_env(config: Config) -> None:
    """Export provider-key compatibility env vars for external CLIs.

    Kyber stores provider secrets in KYBER_* vars/.env. Some external tools
    (e.g. summarize.sh) expect provider-native env names like OPENAI_API_KEY.
    We expose those aliases at runtime without overriding explicit user values.
    """
    _set_env_if_missing("OPENROUTER_API_KEY", config.providers.openrouter.api_key)
    _set_env_if_missing("OPENAI_API_KEY", config.providers.openai.api_key)
    _set_env_if_missing("ANTHROPIC_API_KEY", config.providers.anthropic.api_key)
    _set_env_if_missing("DEEPSEEK_API_KEY", config.providers.deepseek.api_key)
    _set_env_if_missing("GROQ_API_KEY", config.providers.groq.api_key)
    _set_env_if_missing("GEMINI_API_KEY", config.providers.gemini.api_key)
    _set_env_if_missing("GOOGLE_API_KEY", config.providers.gemini.api_key)
    _set_env_if_missing(
        "GOOGLE_GENERATIVE_AI_API_KEY",
        config.providers.gemini.api_key,
    )

    # If the active provider is custom (OpenAI-compatible), export OPENAI_API_KEY
    # as a compatibility fallback for CLIs that only read standard provider names.
    details = config.get_provider_details()
    if bool(details.get("is_custom")):
        api_key = str(details.get("api_key") or "")
        _set_env_if_missing("OPENAI_API_KEY", api_key)
